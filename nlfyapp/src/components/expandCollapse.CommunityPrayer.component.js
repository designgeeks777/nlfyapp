...
//     const position = index % 1 === 0 ? "right" : "left";
//     return (
//       <Item
//         data={props.data}
//         id={item._id}
...
const Item = (props) => {
  ...
  return (
    <>
     ...
     <View style={setFlex}>
        <View style={[containerStyle, { marginLeft: width * 0.02 }]}>
          <Image style={styles.profilePicture} source={getUserProfilePic()} />
          <Text style={[nameStyle,{ marginTop: width * 0.01 }] }>{props.item.raisedBy}</Text>
        </View>
...
export const ExpandCollapseListCommunityPrayer = (props) => {
  ...
  const renderItem = ({ item, index }) => {
    const position = index % 2 === 0 ? "left" : "right";
    //const position = index % 1 === 0 ? "right" : "left";
    //const position = index % 1 === 0 ? "left" : "right";
    return (
      <Item
        data={props.data}
        id={item._id}
